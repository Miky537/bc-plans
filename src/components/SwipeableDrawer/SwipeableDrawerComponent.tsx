...
import { searchTeacher, getRoomPhoto } from "../TeacherSearch/apiCalls";
...
export function SwipeableDrawerComponent({
	...
	const { roomData: ahoj } = useFacultyContext()

	const theme = useTheme();


	// const { data: Photos, refetch } = useQuery({
	// 		queryKey: ['fetchRoomPhotos', mistnostid],
	// 		queryFn: () => getRoomPhotos(mistnostid),
	// 		enabled: !!mistnostid, // Turn off automatic execution
	// 		onSuccess: (data) => {
	...
	useEffect(() => {
		...
		const roomId = ahoj.room_info.mistnost_id; // The ID of the room you're interested in
		console.log("roomId", roomId)
		const photoId = 456; // The ID of the specific photo to fetch
		getRoomPhoto(roomId)
			.then((url: any) => {
				setPhoto(url);
				console.log("yos", url)
			})
			.catch((error: any) => console.error('Failed to load image:', error));
	}, [ahoj.room_info.mistnost_id]);
